...
public:
    ...
    Annihilator(const Vec2<Fixnum>& pos,
                u8 spawn_x,
                u8 spawn_y) :
    ...
    void step() override
    {
        ...
        case 140:
            if (not hflip_) {
                if (auto e = engine().add_object<Supershot>(Vec2<Fixnum>{x() - 20, y() - 2})) {
                    e->set_speed(1, 140);
                }
                if (auto e = engine().add_object<Supershot>(Vec2<Fixnum>{x() - 20, y() - 2})) {
                    e->set_speed(1, 220);
                ...
                if (auto e = engine().add_object<Supershot>(Vec2<Fixnum>{x() + 16, y() - 2})) {
                    e->set_speed(1, 40);
                }
                if (auto e = engine().add_object<Supershot>(Vec2<Fixnum>{x() + 16, y() - 2})) {
        ...
        case 200: {
            ...
            for (int i = 0; i < 4; ++i) {
                if (auto e = engine().add_object<EnemyShot>(Vec2<Fixnum>{x() - 1, y() - 1})) {
                    e->set_speed(0.25f, i * 90);
                }
            }
            if (not hflip_) {
                if (auto e = engine().add_object<Gigashot>(Vec2<Fixnum>{x() - 20, y() - 2})) {
                    auto dir = direction(fvec(position_),
                                         fvec(engine().hero()->position()));
                    e->set_speed({Fixnum(dir.x), Fixnum(dir.y)});
                }
            } else {
                if (auto e = engine().add_object<Gigashot>(Vec2<Fixnum>{x() + 16, y() - 2})) {
                    auto dir = direction(fvec(position_),
                                         fvec(engine().hero()->position()));
                    e->set_speed({Fixnum(dir.x), Fixnum(dir.y)});
                }
